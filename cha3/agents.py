...
class DQNAgent:
    def __init__(self, state_dim: int, action_dim: int, cfg: SimConfig, ddqn: bool = True,
                 device: str | None = None):
        if torch is None:
            raise RuntimeError("PyTorch 未安装，无法训练 DQN/DDQN。请在 handover 环境中安装 torch。")
        self.cfg = cfg
        self.state_dim = state_dim
        self.action_dim = action_dim
        self.ddqn = ddqn
        self.device = torch.device(device or ("cuda" if torch.cuda.is_available() else "cpu"))
        self.online = QNetwork(state_dim, action_dim, cfg.hidden_dim).to(self.device)
        self.target = QNetwork(state_dim, action_dim, cfg.hidden_dim).to(self.device)
    ...
    def update(self) -> AgentStats:
        ...
        # Huber loss 比 MSE 对偶发的大 TD 误差更鲁棒，训练曲线通常更平滑。
